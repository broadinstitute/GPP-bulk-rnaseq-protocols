...
```
Note: do NOT normalize the reads prior to using the DESeq2 library. 

#### 4. Differentially Expressed Genes (DEG) Analysis
##### 4.1 Create Design Matrix 
Refer to this [guide](https://bioconductor.org/packages/release/workflows/vignettes/RNAseq123/inst/doc/designmatrices.html) to understand design matrix terminology and how to create a suitable one for your experiment. The following is an example of the Mean Reference model. Using the Means model in conjunction with `contrasts` is also valid (but would not be compatible with the `apeglm` algorithm in section 4.5).
```{r, design-matrix, message=FALSE}
# In this experiment we combine guide and treatment into one factor
...
Note: as previously mentioned, the `apeglm` algorithm is not compatible with the use of `contrasts`, but `ashr` is. See [extended section on shrinkage estimators](http://bioconductor.org/packages/devel/bioc/vignettes/DESeq2/inst/doc/DESeq2.html#extended-section-on-shrinkage-estimators). We did not explore whether the result is the same between `apeglm` vs `ashr`. 
...
#### LFC vs Baseline Average Expression
The `baseMean` column in the `resLFC` output is the average mean across *all* samples, so we need to calculate the average expression of the reference samples separately. 
```{r, calc-ref-avg, message=FALSE}
resLFC$RefAvg <- rowMeans(reads.matrix.filtered[, annot.minus.outlier$Sample == "untreated_ctrl"]) 
resLFC
```

```{r, mean-diff-plot, message=FALSE}
...
DESEQ.meanDifferencePlot(resLFC, "Untreated sg1 vs Untreated Control")
```
...
The code sample here only shows the result of one comparison - Untreated sg1 vs Untreated control - in order to clearly demonstrate DESeq2 usage. Code optimizations for analyzing the whole experiment is available as a R script. 
